...
/**
 * {@link ChannelHandler}的集合, 用于处理{@link Channel}的入站事件和出站操作。
 * {@link ChannelPipeline} 实现了拦截过滤器模式, 用户可以完全控制事件如何处理及pipeline中 {@link ChannelHandler} 如何彼此交互。
 ...
 *
 * 下面的图表展示了I/O事件如何被 {@link ChannelPipeline}中的 {@link ChannelHandler}s处理。
 ...
 *  +-------------------------------------------------------------------+
 * </pre>
 * 入站事件自下而上的流动, 超过最顶部的inbound handler后, 它将被安静的丢弃, 或在需要时记录日志。
 ...
 * <p>
 *
 * 出站事件是自上而下的方向, 超过最底部的outbound handler之后, 它会被 {@link Channel} 关联的I/O线程处理。
 * 这个I/O线程通常执行实际的输出操作比如 {@link SocketChannel#write(ByteBuffer)}。
